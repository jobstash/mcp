  ...
  async parse(input: FileParserInput): Promise<string> {
    ...
    try {
      ...
      if (run.status !== 'completed') {
        const steps = await this.openai.beta.threads.runs.steps.list(threadId, runId, { limit: 1, order: 'desc' });
        const lastError = steps?.data?.[0]?.last_error;
        throw new Error(`Run failed: ${lastError?.code} - ${lastError?.message}`);
      }
